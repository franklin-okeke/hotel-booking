...
const Navbar = () => {
    ...
    return (
            ...
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "bg-blue-600/50 py-4 md:py-6"}`}>

                {/* Logo */}
                <Link href="/" className={` ${isScrolled ? 'text-blue-600':'text-white'} ${isScrolled && 'font-bold'} flex items-center text-2xl z-99  gap-2`}>
                   Royal Hotels
                </Link>
                    ...
                    {user?(
                      <UserButton>
                        <UserButton.MenuItems>
                            <UserButton.Action label="My Bookings" onClick={()=>router.push('/my-bookings')} />
                        </UserButton.MenuItems>
                      </UserButton>
                    ):
                    (
